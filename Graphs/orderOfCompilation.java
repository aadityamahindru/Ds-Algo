...
public class Main {
   static class Edge {
    ...
    public static void orderOfC(ArrayList<Edge> graph[]){
        boolean visited[]=new boolean[graph.length];
        Stack<Integer> st=new Stack<>();
        for(int v=0;v<graph.length;v++){
            if(!visited[v]){
            orderOfC(graph,visited,st,v);
            }
        }
        while(st.size()>0){
    ...
    public static void orderOfC(ArrayList<Edge> graph[],boolean visited[],Stack<Integer> st,int vtx){
        visited[vtx]=true;
        for(Edge e:graph[vtx]){
            if(!visited[e.nbr]){
                orderOfC(graph,visited,st,e.nbr);
            }
        }
        st.push(vtx);
    }
